...
\begin{figure}[htbp]
  ...
  signal model. (a) and (b) are noise-free. (c) is one sample at the
  uniform-slab floor, where the signal is a fraction of the noise: no
  single spectrum is a detection, and none is meant to be. (d) is the
  object actually predicted --- coadding drives the measured channel
  covariance onto $|S_{ij}|$, while the no-Faraday hypothesis is a flat
  line at unity.}
  \label{fig:data}
\end{figure}
