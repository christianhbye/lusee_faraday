...
map; that is a random walk whose amplitude falls without limit as the
sphere is refined, so it was measuring pixelisation shot noise
...
\begin{equation}
  ...
  \label{eq:walk}
\end{equation}
every cross term averaging to zero. The \emph{normalised} amplitude
therefore has a floor with nothing fitted in it,
\begin{equation}
  \frac{\sqrt{\langle |P|^2\rangle}}{\sum_n |w_n|}
  = \frac{\sqrt{\sum_n |w_n|^2}}{\sum_n |w_n|}
  = N_{\rm eff}^{-1/2},
  \qquad
  N_{\rm eff} \equiv
  \frac{\bigl(\sum_n |w_n|\bigr)^{2}}{\sum_n |w_n|^{2}} ,
  \label{eq:nhalf}
\end{equation}
which for $N$ pixels of comparable weight is the familiar $N^{-1/2}$.

The beam- and emissivity-weighted sky is emphatically not that case,
and the distinction is not cosmetic. $N_{\rm eff}$ is only
\walkNEffLo{} at nside \walkNsideLo{} and \walkNEffHi{} at nside
\walkNsideHi{} --- \walkNEffFracLo\% and \walkNEffFracHi\% of the
pixels present --- because the weight is concentrated on the Galactic
plane and on a few compact sources. Writing $N_{\rm pix}^{-1/2}$ there
understates the floor by a factor \walkFloorGap{} at nside
\walkNsideHi{}, which is enough to make an estimator sitting exactly on
its floor look like one that failed to reach it.
...
to zero under refinement. Concentration slows that decay without
stopping it --- $N_{\rm eff} \propto N_{\rm pix}^{\walkNEffExponent}$
across the nsides above, so the floor falls as
$N_{\rm pix}^{\walkSlopePredicted}$ rather than as
$N_{\rm pix}^{-1/2}$, and Figure~\ref{fig:bridge} measures
$N_{\rm pix}^{\walkSlopeMeasured}$. The direct demonstration is the
acceptance gate of Section~\ref{sec:gates}, where nside $256 \to 2048$
collapses the coherent power by a factor $134$. Those gates weight
every pixel equally, so $N_{\rm eff} = N_{\rm pix}$ there and
Equation~\eqref{eq:nhalf} predicts $64$: the same order, and nothing
like the invariance a physical signal would show.
...
\begin{figure}[htbp]
  ...
  it converges, tracking the dotted line means it is measuring
  $N_{\rm eff}$. That line is Equation~\eqref{eq:nhalf} on the actual
  weights, with no normalisation fitted, and the band is the 16--84\%
  spread of the single \lamsq{} sample plotted, whose $|P|$ is
  Rayleigh --- so a third of all points belong outside it. The real-RM
  points sit at \walkFloorRatioLo--\walkFloorRatioHi$\times$ the
  floor, i.e. inside. Intrinsic angles are set to zero (maximally coherent), so any
  decay is the Faraday phase decorrelating and not the sky's angles.
  Middle and right: the depth distribution that replaces it, and the
  channel covariance that follows. Reading one colour left to right is
  one sky.}
  \label{fig:bridge}
\end{figure}
...
the emissivity geometry. Only one of the perturbations tested below
moves it appreciably --- but that list is not exhaustive, and the one
input this section cannot bound is not the one least likely to matter.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.62\textwidth]{step5_robustness.pdf}
  \caption{Sensitivity of $|S_{ij}|$ to the inputs perturbed here.
  Everything except the emissivity geometry lies on the fiducial
  curve. A spatially varying spectral index is not among them, and is
  the exception discussed in the text.}
...
overall scale or spectral index.

A spatially \emph{varying} index is a different matter, and it is the
input this section leaves unbounded. The weighting extrapolates the
WMAP K polarized map from $23$\,GHz down to the observing band --- a
factor \betaExtrapFactor{} in frequency at 30\,MHz --- and $|w|^2$
carries the emissivity \emph{squared}. Two regions whose indices
differ by only $\Delta\beta = 0.1$ therefore have their relative
weight changed by
$(\nu_{\rm ref}/\nu)^{2\Delta\beta} \approx \betaLeverPointOne$, and
by $\approx \betaLeverPointTwo$ at $\Delta\beta = 0.2$. That is a
reweighting of the sky against itself, which is precisely what $F$
responds to, and the lever arm is long enough that it cannot be
assumed small on the strength of the uniform case above. How much of
it reaches $|S|$ depends on how the index varies \emph{relative to}
the depth structure, and is not measured anywhere in this report.
...
\section{Reading the bracket backwards}
\label{sec:backwards}

Section~\ref{sec:detection} answers the detection question with the
bracket treated as its name suggests: two ends of an interval around
one unknown amplitude. But the two ends do not differ only in
magnitude. Equations~\eqref{eq:slab} and~\eqref{eq:disp} differ by one
power of $\lamsq$, so they are not two guesses at a number --- they are
two different \emph{spectra}. A detection at more than one band
therefore measures something the single-band verdict cannot: which
geometry the interstellar medium sets, which
Section~\ref{sec:bracket} calls the single unknown the science case
turns on.

This section derives what that measurement would demand and stops
there. It quotes a \textbf{requirement, not a capability}. The
systematics that would actually limit it are not modelled anywhere in
this report, so nothing here enters a verdict, and
Table~\ref{tab:detection} is unchanged by it.

\subsection{The floors are two spectra}
\label{sec:twoslopes}

$A_{\rm slab} = 1/(|\phi_{\rm med}|\lamsq) \propto \nu^{2}$ against
$A_{\rm disp} = 1/(2\sigma_{\rm eff}^{2}\lambda^{4}) \propto \nu^{4}$,
so between any two bands the floors predict different amplitude
ratios. Between 30 and 50\,MHz the slab floor rises by
\slopeSlabFiftyThirty{} and the dispersion floor by
\slopeDispFiftyThirty; between 10 and 30\,MHz, by
\slopeSlabThirtyTen{} against \slopeDispThirtyTen.
Figure~\ref{fig:slope} draws both against frequency. Two departures
from the idealised slopes matter, in opposite directions.

\paragraph{The slab floor is not exactly $\nu^{2}$.} $\phi_{\rm med}$
is beam-weighted, and the beam changes with frequency: over
10--50\,MHz it runs $22.5 \to 23.8$\,\rmu, which shifts the computed
ratios from the analytic ones by \slopeDepartSlabFiftyThirty\% and
\slopeDepartSlabThirtyTen\%. Those are small against the factor
separating the floors, but they are \emph{larger than the statistical
error on the ratio} (below), so the analytic slope is not an adequate
model of the slab floor at the precision the measurement reaches.

\paragraph{The dispersion floor is exactly $\nu^{4}$, and that is a
defect rather than a virtue.} Its departure is
\slopeDepartDispFiftyThirty\% and \slopeDepartDispThirtyTen\% --- zero
to the quoted precision, at both band pairs. That is not a statement
about the interstellar medium. It is what
\code{dispersion.amplitude\_bracket} does: $\sigma_{\rm eff}$ enters as
a single hard-coded constant (\sigmaEff\,\rmu) with no beam weighting
and no band dependence, so $A_{\rm disp}$ is a pure power law by
construction. Were $\sigma_{\rm eff}$ built from the map the way
$\phi_{\rm med}$ is, it would drift too. This is the same input
Section~\ref{sec:sigmashape} identifies as the least-scrutinised in the
report, seen from a second direction, and it is the first thing a
serious version of this measurement would have to fix.

\subsection{What the discriminant requires}
\label{sec:sloperequirement}

The observable is a \emph{ratio} between bands, which is why the
discriminant is cheap: it needs no absolute flux scale, and --- as
Section~\ref{sec:pnought} shows it must --- no assumed intrinsic
polarization fraction. Work in logarithms, where the two hypotheses sit
a fixed distance apart,
\begin{equation}
  \Delta = \ln\frac{R_{\rm disp}}{R_{\rm slab}},
  \qquad
  R = \frac{A(\nu_{\rm hi})}{A(\nu_{\rm lo})},
  \label{eq:separation}
\end{equation}
$\Delta = \slopeSepFiftyThirty$ for the 50/30 pair and \slopeSepThirtyTen{}
for 30/10. An $N\sigma$ call needs the total log error on the measured
ratio below $\Delta/N$; the statistical part subtracts in quadrature and
what remains is the budget for everything else,
\begin{equation}
  \sigma_{\rm sys} = \sqrt{(\Delta/N)^{2} - \sigma_{\rm stat}^{2}},
  \qquad
  \sigma_{\rm stat}^{2} = \ln^{2}\!\left(1 + \frac{1}{{\rm SNR}_{\rm lo}}\right)
  + \ln^{2}\!\left(1 + \frac{1}{{\rm SNR}_{\rm hi}}\right).
  \label{eq:slopebudget}
\end{equation}
The SNRs are those achieved \emph{at the slab floor} --- $5\times$ the
detection ratios of Table~\ref{tab:detection}, so \snrSlabThirty{} at
30\,MHz, \snrSlabFifty{} at 50 and \snrSlabTen{} at 10. That choice is
not a convenience, and Section~\ref{sec:asymmetry} is about why.

\begin{table}[htbp]
  \centering
  \caption{The slope discriminant and what it demands. Band-to-band
  amplitude ratios are given with the idealised $\nu^{2}$ / $\nu^{4}$
  values in parentheses. The last two columns are the tolerable
  band-to-band gain error, as a factor, for a $3\sigma$ and a
  $5\sigma$ call. Statistical errors are at the slab floor.}
  \label{tab:slope}
  \begin{tabular}{lccccc}
    \toprule
    Bands & $R_{\rm slab}$ ($\nu^{2}$) & $R_{\rm disp}$ ($\nu^{4}$)
      & $\sigma_{\rm stat}$ & $3\sigma$ & $5\sigma$ \\
    \midrule
    \SlopeRows
    \bottomrule
  \end{tabular}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{step5_slope.pdf}
  \caption{The bracket read backwards. Left: the two floors are two
  spectra, and the gap between them widens with frequency. Lines are
  the analytic power laws anchored on the 30\,MHz computed values,
  markers the computed bracket; the slab markers sit off their line by
  the $\phi_{\rm med}$ drift of Section~\ref{sec:twoslopes} while the
  dispersion markers sit exactly on theirs, for the reason given
  there. Error bars are $1/{\rm SNR}$ at the slab floor and are
  invisible at 50\,MHz. Right: the requirement,
  Equation~\eqref{eq:slopebudget}, against the significance demanded.
  Where a curve reaches unity the statistics alone exhaust $\Delta$ and no
  calibration is good enough --- for 30/10 that is $N = 14.5$, set by
  the 10\,MHz SNR of \snrSlabTen{} and not by any calibration
  property.}
  \label{fig:slope}
\end{figure}

\textbf{The requirement is loose, and it is relative.} A $5\sigma$
discrimination on the 50/30 pair tolerates a band-to-band gain error of
a factor \slopeFactorFiveFiftyThirty{} (\slopeBudgetFiveFiftyThirty\%);
$3\sigma$ tolerates \slopeFactorThreeFiftyThirty{}
(\slopeBudgetThreeFiftyThirty\%). The 30/10 pair is looser still ---
\slopeFactorFiveThirtyTen{} and \slopeFactorThreeThirtyTen{} --- because
its lever arm in $\lamsq$ is longer, though it pays for that with the
10\,MHz SNR. Statistics contribute \slopeStatFiftyThirty\% and
\slopeStatThirtyTen\% and are nowhere near binding. \textbf{This is
therefore a systematics specification and not a sensitivity one},
which is the useful thing about it: closing a sensitivity gap costs
observing time as its square (Section~\ref{sec:detection}), while a
factor-\slopeFactorFiveFiftyThirty{} relative gain requirement is an
ordinary calibration statement.

It is stated as a floor on what must be controlled, not as an achieved
error. Relative gain is one term; a spatially varying spectral index is
another, and Section~\ref{sec:inputsens} bounds only the effect of an
overall index on the template shape, not the effect of index structure
on a band-to-band amplitude ratio. Neither is modelled here. Whether
the true budget is comfortably inside a factor
\slopeFactorFiveFiftyThirty{} is an instrument question this report
cannot answer, and until it is answered the discriminant is a
specification and not a result.

\subsection{The two branches are not symmetric}
\label{sec:asymmetry}

The slope exists only where there is a detection to take a ratio of,
and by Table~\ref{tab:detection} that is the slab branch alone: at the
dispersion floor the detection ratio is \detSafeDispThirty{} at
30\,MHz and \detSafeDispFifty{} at 50, so there is no measured
amplitude at either band and no ratio to form. The discriminant is
therefore asymmetric, and usefully so:

\begin{itemize}
  \item \textbf{Slab branch.} Detection at 30 and 50\,MHz at
    \detSafeSlabThirty$\times$ and \detSafeSlabFifty$\times$
    threshold. The ratio is then measurable and the $\nu^{2}$ slope is
    a \emph{confirmation} --- an internal consistency check on the
    geometry, available for the price of the calibration above.
  \item \textbf{Dispersion branch.} No detection at any band. The
    discrimination is carried entirely by the non-detection, which
    excludes the slab floor at high confidence without any slope being
    measured at all.
\end{itemize}

So the slope is not the instrument that decides between the floors in
the case where they are hardest to tell apart. It is a check that the
detected case is what it appears to be, and a way of catching an
intermediate geometry --- one whose amplitude sits between the floors
--- which the two-branch reading of Table~\ref{tab:detection} would
otherwise report simply as a weak slab detection.

\subsection{What the slope cannot do}
\label{sec:pnought}

It cannot measure the Faraday strength. The observable is
\begin{equation}
  A_{\rm obs}(\nu) = p_{0}\,A_{\rm floor}(\nu),
  \label{eq:pnought}
\end{equation}
with $p_{0}$ the emission-weighted intrinsic polarization fraction
along the column, and \textbf{a power law's slope carries its exponent,
not its amplitude}. The slope returns the exponent, which is the
geometry; $\phi_{\rm med}$ and $\sigma_{\rm eff}$ live in the
amplitude, where they are exactly degenerate with $p_{0}$. Nothing in
this report determines $p_{0}$: it is not the $0.7$ of optically thin
synchrotron, because the emission is weighted along a column that is
already partly depolarised before the beam sees it.

This also disposes of a tempting shortcut. One cannot measure
depolarisation directly, because the undepolarised state is never
observed at these wavelengths --- Section~\ref{sec:bracket}'s external
screen is suppressed by $e^{-2\sigma^{2}\lambda^{4}}$ with an exponent
of $\approx1.9\times10^{6}$ at 30\,MHz, so a beam-averaged
polarization fraction is not small, it is zero, and so is its
derivative with respect to $\sigma$. What is observed is the residual
of Equation~\eqref{eq:pnought}, and converting that residual to a
Faraday strength requires $p_{0}$ from outside.

\subsection{The shape closes it, and that is the work}
\label{sec:shapecloses}

There is a route, and it uses machinery this report already has. The
predicted template \emph{shape} is amplitude-free by construction ---
Section~\ref{sec:depthdist} normalises it, and every shape statistic
here is a CDF quantile that never references the peak --- yet it is
built from the same column depths that set $\phi_{\rm med}$. The
90\%-mass knee is \kneeThirty\,\rmu{} at 30\,MHz, \kneeFifty{} at 50
and \kneeTen{} at 10. So the shape constrains the Faraday strength
with no amplitude information at all, and
Equation~\eqref{eq:pnought} then reads in the other direction:
\begin{equation}
  \text{shape} \;\longrightarrow\; \phi_{\rm med},
  \qquad
  \frac{A_{\rm obs}}{A_{\rm floor}(\phi_{\rm med})}
  \;\longrightarrow\; p_{0}.
  \label{eq:closure}
\end{equation}
Read forwards it measures $p_{0}$; read as a consistency check it tests
whether an assumed $p_{0}$ and the measured amplitude agree with the
depth structure the same data show. \textbf{Neither is built here.}
Equation~\eqref{eq:closure} needs an error model on the shape-derived
$\phi_{\rm med}$, which needs the coherence question of
Section~\ref{sec:tilt} settled, since the tilt moves the knee to
\tiltKneeThirty\,\rmu{} at 30\,MHz --- a factor of five. That is a
larger effect than everything else in this section combined, and it is
the reason the closure is named here and not evaluated. It enters no
verdict.
...
Claim & Test \\
\midrule
The walk's floor, Eq.~\eqref{eq:nhalf}
  & \code{test\_effective\_pixel\_count\_is\_the\_random\_walk\_floor} \\
...
The $\nu^2$/$\nu^4$ slopes, \S\ref{sec:twoslopes}
  & \code{test\_floor\_slopes\_are\_nu2\_and\_nu4} \\
$\phi_{\rm med}$ drift off $\nu^2$, \S\ref{sec:twoslopes}
  & \code{test\_slab\_slope\_departs\_by\_exactly\_the\_phi\_med\_drift} \\
Slope budget, Eq.~\eqref{eq:slopebudget}
  & \code{test\_slope\_requirement\_is\_a\_relative\_gain\_budget} \\
...
\begin{verbatim}
    ...
    uv run python scripts/step5_intuition.py     # needs data/
