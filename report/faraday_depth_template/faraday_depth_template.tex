...
Two results are methodological. First, the earlier diffuse calculation
summed the Faraday phasor pixel by pixel over a HEALPix map; that is a
random walk whose amplitude falls as $N_{\rm pix}^{-1/2}$, so it was
measuring pixelisation shot noise rather than sky. Binning the beam-
and emissivity-weighted sky \emph{in Faraday depth first} and
transforming second gives a distribution $F(\phi)$ whose shape is
invariant under refinement. Second, that distribution can be read out
in Faraday depth $\phi$ or in delay $\tau$ with no loss: the two are
related by a monotonic map, the chirp that distinguishes them smears by
only \chirpPctThirty--\chirpPctFifty\% of the template's own extent, and the
detection statistic is identical in either basis.
...
between pixels, and so is $2\phi_n\lamsq$ modulo $2\pi$: at nside 512
the depth changes by order $10^{3}$\,\rmu{} between neighbours, so at
30\,MHz ($\lamsq = 99.86\,\mathrm{m^2}$) the phase advances hundreds of
turns from one pixel to the next. Then
\begin{equation}
...
The sum in Equation~\eqref{eq:pixelsum} is computed correctly. It is
simply the wrong estimator for a quantity whose phases decorrelate
...
\citep{burn1966,brentjens2005}. What changes is the order: binning is a
histogram operation, and a histogram's shape is stable under refinement
because refining splits a pixel's weight among sub-pixels of nearly the
same depth. The shot noise of Equation~\eqref{eq:nhalf} does not
disappear --- it is confined to the normalisation, which is exactly the
quantity this report declines to predict.
...
\subsection{The incoherent limit}
\label{sec:identity}
...
term is $F$ convolved with $|R|^2$. The same sum that fails as an
\emph{amplitude} estimator succeeds as a \emph{power} estimator: the
shot noise of Equation~\eqref{eq:walk} is precisely the diagonal term
...
\section{One signal, two bases: Faraday depth and delay}
\label{sec:bases}
...
Modelling the signal as a Gaussian field of known normalised shape
$\hat H$ and unknown amplitude $A$, its frequency covariance is
$S_{ij} = \sum_b \hat H_b e^{2i\phi_b(\lamsq_i-\lamsq_j)}$ with
$\sum_b \hat H_b = 1$, and for $M$ LST samples and $n$ coherent nights
\begin{equation}
...
$\mathbf{1.160}$. That ratio, and not any comparison to a closed form,
is the cost of the overlap.
...
\begin{itemize}
  \item \textbf{At the uniform-slab floor the signal is detectable} ---
    $\detSafeSlabThirty\times$ threshold at 30\,MHz,
    $\detSafeSlabFifty\times$ at 50\,MHz, $\detSafeSlabTen\times$ at
    10\,MHz.
...
cut in delay as much as in depth (Section~\ref{sec:bases}); the safe cut
...
\section{The template, and how robust its shape is}
\label{sec:robust}

Figure~\ref{fig:family} is the template that Equation~\eqref{eq:mf}
matches against. The roll-off statistic is the \textbf{90\%-mass
...
\begin{figure}[htbp]
  ...
  geometries of Section~\ref{sec:pushforward}. Colour is the geometry
  and linestyle the treatment. $k\to-1$ is $\delta(\phi)$ by
  construction and is marked rather than drawn.}
  \label{fig:family}
\end{figure}
...
Step 4, delay-space signature & \textbf{superseded} & \S\ref{sec:bases}, \S\ref{sec:detection} \\
\bottomrule
\end{tabular}
\end{center}
...
basis: Section~\ref{sec:bases} finds the delay axis it used to be
